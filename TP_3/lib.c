...
int getDataFilms(eMovie* pelis,int length)
{
    ...
    fflush(stdin);
    gets(auxDescripcion);
    while(strlen(auxDescripcion)>400)
    {
...
int cargarBinario(eMovie* pelis,int length)
{
    ...
    fread(pelis,sizeof(eMovie),length,pFile); //LEO Y GUARDO
    retorno=0;
    printf("\n Archivo cargado con exito!\n\n");
    system("pause");
    fclose(pFile); // CIERRO EL ARCHIVO ABIERTO
    return retorno;
}
...
/**\brief Genera y lista las peliculas en una pagina web a partir de un array
